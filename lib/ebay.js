...
function marketListingFilter() {
  return (
    process.env.MARKET_LISTING_FILTER ||
...
export async function marketAspectCount(query, { categoryId, sampleLimit = 50 } = {}) {
  ...
  // Count sellable listings only; this also makes the aspect Model counts sellable.
  params.set("filter", marketListingFilter());
  ...
  // A page of items for average-price sampling (summary-level filters only).
  ...
  const items = (data.itemSummaries || [])
    .filter((it) => {
      ...
      const cats = (it.leafCategoryIds || []).map(String);
      if (cats.length && !cats.some((c) => ALLOWED_LEAF.has(c))) return false;
      if (COSMETIC_RE.test(it.condition || "")) return false;
      return true;
    })
    .map((it) => ({
      source: "ebay",
...
export async function aspectFilterTotal(query, { categoryId, modelValue } = {}) {
  ...
  const qs =
    ...
    `&filter=${encodeURIComponent(marketListingFilter())}`;
  try {
