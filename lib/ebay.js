...
export async function search(query, { limit = 10, enrichCount = 10 } = {}) {
  ...
  // Restrict to eBay's phone + tablet categories only (excludes cases/accessories,
  // which live in separate accessory categories):
  //   9355   = Cell Phones & Smartphones
  //   171485 = Tablets & eBook Readers
  const CATEGORY_IDS = process.env.EBAY_CATEGORY_IDS || "9355,171485";
  const url = `${SEARCH_URL}?q=${encodeURIComponent(query)}&limit=${limit}&category_ids=${encodeURIComponent(CATEGORY_IDS)}`;
