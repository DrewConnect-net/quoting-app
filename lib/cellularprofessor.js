...
async function getCatalog() {
  ...
  try {
    ...
    if (!res.ok) throw new Error(`Cellular Professor API failed (${res.status})`);
...
export async function search(query, { limit = 10 } = {}) {
  if (!isEnabled()) return { source: "cellularprofessor", enabled: false, items: [] };
  ...
  return { source: "cellularprofessor", enabled: true, items };
}

export const meta = { id: "cellularprofessor", label: "Cellular Professor (wholesale)", isEnabled };
