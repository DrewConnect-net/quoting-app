...
// Map a Cellular Professor condition tag to the grade we surface. Per the grading
// spec, ONLY three tiers are listed; everything else returns null and is dropped:
//   "New" / "NoB"            -> "New"
//   "Premium ..."            -> "Premium"
//   grade 8 or 9 (8/8X/9/9X, -> "AB+"
//     incl. BOX / Other / WiFi / Cellular variants)
// Lower cosmetic grades (6, 4) are excluded entirely. The Cellular/WiFi part is
// just connectivity (already shown in the product title), so it is not surfaced.
function gradeLabel(cond) {
  const c = String(cond || "").trim();
  if (/^new\b/i.test(c) || /^nob\b/i.test(c)) return "New";
  if (/^premium/i.test(c)) return "Premium";
  if (/^[89]/.test(c)) return "AB+";
  return null;
}
...
export async function search(query, { limit = 10 } = {}) {
  ...
  // Keep only the exact model AND only the grades we surface (New / Premium / AB+).
  const matched = filterByModel(query, inStock).matched
    .filter((p) => gradeLabel(p.condition) !== null);
  ...
  const items = matched.slice(0, Math.max(Number(limit) || 10, 60)).map((p) => ({
    ...
    currency: "USD",
    grade: gradeLabel(p.condition),                     // "New" | "Premium" | "AB+"
